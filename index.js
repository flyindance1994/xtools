...
if (options.x) {
    if (Object.keys(options.x).length !== 2) {
        console.error("ERROR:only 2 str can be accepted")
        return
    }

    let arrA = new Array()
    let arrB = new Array()
    arrA = options.x[0].split(",")
    arrB = options.x[1].split(",")

    console.log("common items A & B :")
    console.log(arrA.filter(item => arrB.includes(item)))

    console.log("item of A not in B:")
    console.log(arrA.filter(item => !arrB.includes(item)))

    console.log("item of B not in A:")
    console.log(arrB.filter(item => !arrA.includes(item)))
}
